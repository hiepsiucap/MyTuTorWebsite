...
const EachTuTorV2 = ({ t }) => {
  ...
  const [hidden, gethidden] = useState(false);
  const deleteById = async () => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/v1/listlike/${t._id}`,
        {
          method: "DELETE",
          withCredntials: true,
          credentials: "include",
      ...
      if (response.ok) {
        gethidden(true);
        NotificationManager.success("Xoá thành công", "Xoá thành công");
      }
    } catch (err) {}
  };
  return (
    <>
      {!hidden ? (
        <Link
          to={link}
          className="flex flex-col overflow-hidden  rounded-xl bg-white shadow-md pb-5 border border-gray-300 relative"
        >
          <div
            onClick={(e) => {
              e.preventDefault();
            ...
            className="  absolute top-0 right-0 z-30 opacity-75"
          >
            <svg
...
"
          ...
          <div className="">
            <img
              src={t.ava}
              alt=""
              className=" rounded-lg border h-80 w-full scale-105"
            />
          </div>
          <div className="flex flex-col py-3 px-3 pt-6">
            <h2 className=" text-xl font-bold">{t.name}</h2>
            <h5 className=" text-sm font-medium h-16">{t.university}</h5>
          </div>
          <div className=" felx flex-col border-l border-gray-200 px-3 space-y-2">
            <div className="flex space-x-1 items-end">
              <h5 className=" font-bold leading-relaxed text-lg">
